...
export const addProduct = createAsyncThunk('addProduct', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/product/${localStorage.getItem('userId')}`, {
            method: "POST",
            body: data,
            credentials: "include"
        })
...
export const getProducts = createAsyncThunk('getProducts', async (data={}, {rejectWithValue}) => {
    try{
        ...
        const response = await fetch(`http://localhost:3002/product?${queryString}`, {
            method: "GET",
            credentials: "include"
        })
        if(!response.ok){
...
export const getHomeProducts = createAsyncThunk('getHomeProducts', async (data={}, {rejectWithValue}) => {
    try{
        let queryString = createQueryParams(data)
        const response = await fetch(`http://localhost:3002/product/home?${queryString}`, {
            method: "GET",
            credentials: "include"
        })
        if(!response.ok){
...
export const getProductDetails = createAsyncThunk('getProductDetails', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/product/${data}`, {
            method: "GET",
            credentials: "include"
        })
        if(!response.ok){
...
export const editProduct = createAsyncThunk('editProduct', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/product/${localStorage.getItem('productId')}/${localStorage.getItem('userId')}`, {
            method: "PUT",
            body: data,
            credentials: "include"
        })
...
export const deleteProduct = createAsyncThunk('deleteProduct', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/product/${data}/${localStorage.getItem('userId')}`, {
            method: "DELETE",
            credentials: "include"
        })
        if(!response.ok){
