...
export const addCategory = createAsyncThunk('addCategory', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/admin/category/${localStorage.getItem("adminId")}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
...
export const editCategory = createAsyncThunk('editCategory', async (data, {rejectWithValue}) => {
    try{
        const response = await fetch(`http://localhost:3002/admin/category/${data.categoryId}/${localStorage.getItem("adminId")}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
